// All content is stored and used from this file
// Recomend multiple parts are created as separate files
// and imported to keep this file streamlined

const Content = {
	header: {
		title: 'Photo album title',
		intro: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Tempora distinctio maxime perferendis earum, nesciunt, ullam inventore minus quaerat, omnis sequi deleniti. Cum assumenda itaque obcaecati aperiam, provident, vel ipsum quo.',
		// map: {
		// 	country: 'japan',
		// 	scale: 2,
		// 	circle: 0,
		// 	origin: [100, 100],
		// },
	},

	parts: [
	{
			title: '',
			text: '',
			link: '',
			hide: true,
			map: {
				country: 'united kingdom',
				scale: 0,
				circle: 0,
				origin: [100, 100],
			},
		},
		{
			title: 'UK',
			text: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Laborum explicabo quidem eius commodi in assumenda reprehenderit, dolor saepe corrupti exercitationem doloremque, ullam dolorum magnam harum vel molestiae est! Aut, at?',
			link: '#',
			map: {
				country: 'united kingdom',
				scale: 1,
				circle: 0,
				origin: [100, 100],
			},
		},
		{
			title: 'Peru 2008',
			text: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Laborum explicabo quidem eius commodi in assumenda reprehenderit, dolor saepe corrupti exercitationem doloremque, ullam dolorum magnam harum vel molestiae est! Aut, at?',
			link: '#',
			map: {
				country: 'peru',
				scale: 0.8,
				circle:0,
				origin: [100, 100],
			},
		},
		{
			title: 'Bolivia 2008',
			text: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Laborum explicabo quidem eius commodi in assumenda reprehenderit, dolor saepe corrupti exercitationem doloremque, ullam dolorum magnam harum vel molestiae est! Aut, at?',
			link: '#',
			map: {
				country: 'bolivia',
				scale: 0.8,
				circle:0,
				origin: [100, 100],
			},
		},
		{
			title: 'Chilé 2008',
			text: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Laborum explicabo quidem eius commodi in assumenda reprehenderit, dolor saepe corrupti exercitationem doloremque, ullam dolorum magnam harum vel molestiae est! Aut, at?',
			link: '#',
			map: {
				country: 'chile',
				scale: 0.8,
				circle:0,
				origin: [100, 100],
			},
		},
		{
			title: 'China & Tibet 2009',
			text: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Laborum explicabo quidem eius commodi in assumenda reprehenderit, dolor saepe corrupti exercitationem doloremque, ullam dolorum magnam harum vel molestiae est! Aut, at?',
			link: '#',
			map: {
				country: 'china',
				scale: 0.8,
				circle:0,
				origin: [100, 100],
			},
		},
		{
			title: 'Switzerland 0000',
			text: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Laborum explicabo quidem eius commodi in assumenda reprehenderit, dolor saepe corrupti exercitationem doloremque, ullam dolorum magnam harum vel molestiae est! Aut, at?',
			link: '#',
			map: {
				country: 'switzerland',
				scale: 0.8,
				circle:0,
				origin: [100, 100],
			},
		},
		{
			title: 'Nepal 2008',
			text: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Laborum explicabo quidem eius commodi in assumenda reprehenderit, dolor saepe corrupti exercitationem doloremque, ullam dolorum magnam harum vel molestiae est! Aut, at?',
			link: '#',
			map: {
				country: 'nepal',
				scale: 0.8,
				circle:0,
				origin: [100, 100],
			},
		},
		{
			title: 'Tanzania 2010',
			text: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Laborum explicabo quidem eius commodi in assumenda reprehenderit, dolor saepe corrupti exercitationem doloremque, ullam dolorum magnam harum vel molestiae est! Aut, at?',
			link: '#',
			map: {
				country: 'tanzania',
				scale: 0.8,
				circle:0,
				origin: [100, 100],
			},
		},
		{
			title: 'UK 0000',
			text: 'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Laborum explicabo quidem eius commodi in assumenda reprehenderit, dolor saepe corrupti exercitationem doloremque, ullam dolorum magnam harum vel molestiae est! Aut, at?',
			link: '#',
			map: {
				country: 'united kingdom',
				scale: 0.8,
				circle:0,
				origin: [100, 100],
			},
		},

	],
}

export default Content
